...
def main():
    ...
    parser.add_argument("--password", required=True, help="specify user password")
    parser.add_argument("--host", default="localhost:8001", help="specify host")
    parser.add_argument("--https", default=False, type=bool, help="use https or not")
    parser.add_argument("--timeout", default="5", type=int, help="timeout in seconds")
    parser.add_argument(
        "--debug", default=True, type=bool, help="output debug information"
    )

    args = parser.parse_args()
    okHttpClient = ApiClient(
        args.email, args.password, args.host, args.https, args.timeout, args.debug
    )
    ...
    while True:
        try:
            ...
            okHttpClient.post(
                "/api/orders",
                dict(
                    price=randomPrice(world_price, "BUY"),
            ...
            okHttpClient.post(
                "/api/orders",
                dict(
                    price=randomPrice(world_price, "SELL"),
                    quantity=randomQuantity(),
                    direction="SELL",
                ),
            )
